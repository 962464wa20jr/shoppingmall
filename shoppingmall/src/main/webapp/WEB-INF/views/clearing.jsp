...
<%Map<Goods,Integer> goodsMap=(Map<Goods,Integer>)session.getAttribute("clearGoodList"); %>
<%Map<Goods,Float> goodsSumMap=(Map<Goods,Float>)session.getAttribute("littleSum"); %>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>TIMA超市</title>
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath }/res/static/css/public.css"/>  
...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath }/res/static/css/main.css"/>
        ...
        <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath }/res/static/css/checkOut.css" />
</head>
 ...
 <!--顶部快捷菜单-->
    ...
    <div class="site-nav w1200">
      ...
      <div class="sn-quick-menu">
        ...
        <input type="hidden" value="${currentUser.getId() }" id="cuserId"/>
...
<div class="box-bd">
    <div class="clearfix xm-address-list" id="checkoutAddrList">
             ...
             <span class="iconfont icon-add"><img src="${pageContext.request.contextPath }/res/static/img/add_cart.png" /></span> 
            使用新地址
        </div>
            </div>
    ...
    <div class="xm-edit-addr-box" id="J_editAddrBox">
        <div class="bd">
            ...
            <div class="item">
                ...
                <input type="text" name="userAddress[tag]" id="Tag" class="input" placeholder="地址标签：如'家'、'公司'。限5个字内'" ></input>
                <p class="tip-msg tipMsg"></p>
            </div>
        </div>
    ...
    <div class="box-bd">
        ...
        <div class="invoice-info nvoice-info-1" id="checkoutInvoiceElectronic" style="display:none;">
            ...
            <ul class="type clearfix J_invoiceType">
                ...
                <li class="">
                     <input  class="invoiceType" type="radio" name="Checkout[invoice_type]" value="2" ></input>
                     单位
                </li>
            </ul>
                ...
                <div id="checkoutGoodsList" class="checkout-goods-box">
                    ...
                    <div class="box-bd">
                        <dl class="checkout-goods-list">
                            ...
                            <c:forEach var="good" items="${clearGoodList }" varStatus="goodStatus">
                             <dd class="item clearfix">
                                <div class="item-row">
                                    <div class="col col-1">
                                        <div class="g-pic">
                      
                                            <img src="${pageContext.request.contextPath }${good.key.getImage()}" width="40" height="40" />
                                        </div>
                                        <div class="g-info">
                                         <a href="#">
                                         ${good.key.getGoodsName()}
                                         </a>
                                                                                                                                </div>
                                    </div>
                
                                    <div class="col col-2"  >${good.key.getPrice() }</div>
                                    <div class="col col-3" >${good.value }</div>
                                    <div class="col col-4"  >${littleSum.get(good.key)}</div>
                                </div>
                            </dd>
                            </c:forEach>
                        ...
                        <div class="checkout-count clearfix">
                            ...
                            <div class="checkout-price">
                                <ul>
                                    ...
                                    <li>
                                       订单总额：<span>${bigSum }元</span>
                                    </li>
                                    <li>
                                        活动优惠：<span>-0元</span>
                                        <script type="text/javascript">
                                            checkoutConfig.activityDiscountMoney=0;
                                            checkoutConfig.totalPrice=${bigSum};
                                        </script>
                                    </li>
                                    <li>
                                ...
                                <p class="checkout-total">应付总额：<span><strong id="totalPrice">${bigSum }</strong>元</span></p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <!--S 保障计划 产品选择弹框 -->
                ...
                <div class="checkout-confirm">
                     ...
                     <input type="button" class="btn btn-primary" value="立即下单" id="checkoutToPay" onclick="ltry()" />
                                     </div>
            </div>
        </div>
...
<script id="newAddrTemplate" type="text/x-dot-template">
        <dl class="item selected" data-isnew="true" data-consignee="{{=it.consignee}}" data-tel="{{=it.tel}}" data-province="{{=it.province}}" data-provincename="{{=it.provinceName}}" data-city="{{=it.city}}" data-cityname="{{=it.cityName}}" data-county="{{=it.county}}" data-countyname="{{=it.countyName}}" data-zipcode="{{=it.zipcode}}" data-street="{{=it.street}}" data-tag="{{=it.tag}}">
            <dt>
                <strong class="itemConsignee" id="shouName">{{=it.consignee}}</strong>
                {{? it.tag }}
                    <span  class="itemTag tag">{{=it.tag}}</span>
                {{?}}
            </dt>
            <dd>
                <p class="tel itemTel" id="shouTel">{{=it.tel}}</p>
                <p  class="itemRegion" id="shouAddr">{{=it.provinceName}} {{=it.cityName}} {{=it.countyName}}</p>
                <p  class="itemStreet" id="shouAddr2">{{=it.street}} ({{=it.zipcode}})</p>
...
<!-- 保险弹窗 -->
    ...
    <script src="${pageContext.request.contextPath }/res/static/js/base.min.js"></script>
        ...
        <script type="text/javascript" src="${pageContext.request.contextPath }/res/static/js/address_all.js"></script>
<script type="text/javascript" src="${pageContext.request.contextPath }/res/static/js/checkout.min.js"></script> 
</div>
 ...
 <!--收货地址body部分结束-->
   ...
   <!--   <script type="text/javascript">
     function like(){
    	 alert("到这来了");
    	 var addr1=$('#shouAddr').html().spilt(' ').join('');
    	 var addr2=$('#shouAddr2').html();
    	 $.ajax({
    		url:"addOrder",
    		type:"POST",
    		contentType:"application/json;charset=utf-8",
    		data:JSON.stringify({
    			reciever_name:$('#shouName').html(),
    			address:addr1+addr2,
    			phoneNumber:$('#shouTel').html(),
    			user_id:$('#cuserId').val()
    		}),
    		success:function(result){
    			if(result!=null){
    				window.href.location="pay";
    			}
    			else{
    				alert("失败了");
    			}
  ...
  function ltry(){
	  alert("开始")
	  var receiverName=$('#shouName').html();
	  
      var addr1=$('#shouAddr').html().replace(/\s*/g,"");
      
      var addr2=$('#shouAddr2').html();
      
      var tel=$('#shouTel').html();
      
      var userid=$('#cuserId').val();
      
      var realAddr=addr1+addr2;
      ...
      $.ajax({
  		  ...
  		  success:function(result){
  			  if(result!=null){
  				 //layer.msg("下单成功");
  				 alert("成功");
  				window.location.href="<%=path%>/order/pay";
  			  }
  			  else{
  				  alert("1失败");
  				  layer.msg("下单失败");
  ...
  <!--  <script type="text/javascript">
   layui.use(['jquery','form'],function(){
    ...
    <script src="${pageContext.request.contextPath }/res/static/js/index.js" type="text/javascript"></script>
